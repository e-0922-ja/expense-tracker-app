...
CASE WHEN uu.id = user_id AND f."statusId" = 1 THEN NULL
     WHEN uu.id = user_id AND f."statusId" = 2 THEN uf."firstName"
     ELSE uu."firstName" END AS "firstName",
CASE WHEN uu.id = user_id AND f."statusId" = 1 THEN NULL
     WHEN uu.id = user_id AND f."statusId" = 2 THEN uf."lastName"
...
ORDER BY
    f."statusId" DESC, f."registeredAt" DESC;
END;
$function$
;
